...
Various calculations on mutation frequencies for the graphical abstract
...
mut <- fread("mc3.v0.2.8.PUBLIC.xena.txt", header=T, sep="\t") #all mutated genes, use the same source as rest of the genome to avoid pre-processing bias#
mut <- as.data.frame(mut)
mut <- subset(mut, !mut$gene %in% unique(m$gene))
mut <- merge(mut,clin,by="sample")
mut <- unique(mut)

m2 <- subset(m, m$cancer.type.abbreviation %in% nab)
m3 <- subset(mut, mut$cancer.type.abbreviation %in% nab)

library(goseq)
res <-list()
res2 <- list()
res3 <- list()
for(i in unique(m2$cancer.type.abbreviation)){
  
  print(i)
  
  z <- subset(m2, m2$cancer.type.abbreviation==i)
  z2 <- subset(m3, m3$cancer.type.abbreviation==i)
  s <- z
  s2 <- z2
  
  z <- as.data.frame(table(z$gene))
  z2 <- as.data.frame(table(z2$gene))
  z$l<- getlength(z$Var1,'hg19','geneSymbol')
  z2$l<- getlength(z2$Var1,'hg19','geneSymbol')
  z$norm <- z$Freq/z$l
  z2$norm <- z2$Freq/z2$l
  
  res[[i]] <- data.frame(tumor.type=i,avg.in.mat=mean(na.omit(z$norm)),avg.in.rest=mean(na.omit(z2$norm)),eval=ifelse(mean(na.omit(z$norm))>=mean(na.omit(z2$norm)),"bigger or equal","lesser"))
  
  s <- paste(s$gene,s$start,s$end,s$effect,s$Amino_Acid_Change,sep="_")
  s2 <- paste(s2$gene,s2$start,s2$end,s2$effect,s2$Amino_Acid_Change,sep="_")
  s <- as.data.frame(table(s))
  s2 <- as.data.frame(table(s2))
  tot <- nrow(s)
  tot2 <- nrow(s2)
  
  #same mutations in at least 2 or 5 patients#
  s_cut <- s[s$Freq >=2,]
  s_cut2 <- s2[s2$Freq >=2,]
  s_cut3 <- s[s$Freq >=5,]
  s_cut4 <- s2[s2$Freq >=5,]
  
  df <- data.frame(tumor.type=i, recurrent.2.in.mat=nrow(s_cut), recurrent.2.in.rest=nrow(s_cut2), recurrent.5.in.mat=nrow(s_cut3), recurrent.5.in.rest=nrow(s_cut4))
  res2[[i]] <- df
  
  df2 <- df
  df2[,2] <- round((df2[,2]/tot)*100,3)
  df2[,3] <- round((df2[,3]/tot2)*100,3)
  df2[,4] <- round((df2[,4]/tot)*100,3)
  df2[,5] <- round((df2[,5]/tot2)*100,3)
  res3[[i]] <- df2
  
}

res <- bind_rows(res)
res$avg.in.mat <- formatC(res$avg.in.mat, format = "e")
res$avg.in.rest <- formatC(res$avg.in.rest, format = "e")
res2 <- bind_rows(res2)
res3 <- bind_rows(res3)

setwd(dir.res)
fwrite(res, "normalized freq of mutation by length_matrisome and rest.txt",sep="\t",row.names = F, quote = F)
fwrite(res2, "recurrency of mutations_matrisome and rest.txt",sep="\t",row.names = F, quote = F)
fwrite(res3, "recurrency of mutations_frequency_matrisome and rest.txt",sep="\t",row.names = F, quote = F)
